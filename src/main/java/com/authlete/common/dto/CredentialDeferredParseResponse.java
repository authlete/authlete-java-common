 ...
 * the deferred credential request is invalid. In this case, the implementation
 ...
 * response should be 400 and {@code application/json}, respectively. The value
 * of the {@code responseContent} parameter can be used as the message body of
 * the error response as it conforms to the specification of "Deferred
 ...
 * deferred credential endpoint should return an error response to the request
 * sender. The HTTP status code of the error response should be 401. The value
 * of the {@code responseContent} parameter can be used as the value of the
 ...
 *
 * <p>
 * Note that the implementation of the deferred credential endpoint should call
 * the {@code /auth/introspection} API to check whether the access token is
 * valid BEFORE calling the {@code /vci/deferred/parse} API. The validation on
 * the access token by the {@code /vci/deferred/parse} API is limited and not
 * exhaustive. For example, the {@code /vci/deferred/parse} API does not check
...
public class CredentialDeferredParseResponse extends ApiResponse
{
     ...
     * to the specification of "Deferred Credential Error Response".
     * </p>
     *
     * @return
     ...
     * conforms to the specification of "Deferred Credential Error Response".
     * </p>
     *
     * @param content
     *         The content of the response that should be returned to the
