...
/**
 * Response from Authlete's {@code /vci/nonce} API.
 *
 * <p>
 ...
 *
 * </p>
 * In this case, the implementation of the nonce endpoint of the credential
 * issuer should return an HTTP response with the HTTP status code
 * "{@code 200 OK}" and the content type "{@code application/json}". The
 * message body of the response has been prepared by Authlete's {@code /vci/nonce}
 * API and it is available as the
 * <code>{@link #getResponseContent() responseContent}</code> response parameter.
 * </p>
 *
 * <p>
 * The implementation of the nonce endpoint can construct an HTTP response by
 * doing like below.
 * </p>
 *
 ...
 * >OpenID for Verifiable Credential Issuance 1.0</a> specification requires
 * the response to include the {@code Cache-Control} header including the
 * value {@code no-store}.
 * </p>
 *
 ...
 * has been prepared by Authlete's {@code /vci/nonce} API and it is available
 ...
 *
 * </p>
 * In this case, a simple implementation of the nonce endpoint would return
 ...
 * {@code /vci/nonce} API and it is available as the <code>{@link #getResponseContent()
 * responseContent}</code> response parameter.
 * </p>
 *
 * <p>
 * Such simple implementation of the nonce endpoint can construct an HTTP response
 * by doing like below.
 * </p>
 *
 * <pre style="border: solid 1px black; padding: 0.5em;">
...
public class CredentialNonceResponse extends ApiResponse
{
    ...
    /**
     * The next action that the implementation of the nonce endpoint
    ...
    {
        /**
         * A nonce response has been prepared successfully. The implementation
         * of the nonce endpoint should return an HTTP response with the HTTP
         * status code "{@code 200 OK}" and the content type
         ...
         * implementation of the nonce endpoint should return an HTTP response
         * with the HTTP status code "{@code 404 Not Found}" and the content
         * type "{@code application/json}" to indicate that the endpoint should
         * not be accessed.
         */
        NOT_FOUND,
         ...
         * not been set up properly yet. A simple implementation of the nonce
    ...
    /**
     * Get the next action that the implementation of the nonce endpoint should
     * take after getting a response from Authlete's {@code /vci/nonce} API.
     *
     * @return
     *         The next action.
     */
    ...
    /**
     * Set the next action that the implementation of the nonce endpoint should
     * take after getting a response from Authlete's {@code /vci/nonce} API.
     *
     * @param action
     *         The next action.
     ...
     *         {@code this} object.
     */
    public CredentialNonceResponse setAction(Action action)
    {
        this.action = action;
    ...
    /**
     * Get the content that the implementation of the nonce endpoint should use
    ...
    /**
     * Set the content that the implementation of the nonce endpoint should use
     * when it constructs a response.
     *
     * @param content
     *         The response content in the JSON format.
    ...
    /**
     * Get the {@code c_nonce} value generated by the {@code /vci/nonce} API.
     *
     * @return
     *         The generated {@code c_nonce} value.
     */
    ...
    /**
     * Set the {@code c_nonce} value generated by the {@code /vci/nonce} API.
     *
     * @param cnonce
     *         The generated {@code c_nonce} value.
     *
